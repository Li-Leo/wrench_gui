...
void SCREEN_RefreshScreen(void)
{
    ...
    while(uiChangedPageIndex <= s_stLCDBuffer.stUpdateArea.uiEndPageIndex)
    {
        ...
        OLED_SetCursor_ex(s_stLCDBuffer.stUpdateArea.uiStartColumnIndex, uiChangedPageIndex);
        ...
        while(uiChangedColumnIndex <= s_stLCDBuffer.stUpdateArea.uiEndColumnIndex)
        {
            ...
            OLED_WriteData(s_stLCDBuffer.auiDisplayCache[uiChangedColumnIndex][uiChangedPageIndex], 1);
